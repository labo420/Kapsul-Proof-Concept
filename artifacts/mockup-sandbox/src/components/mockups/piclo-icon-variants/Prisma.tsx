export function Prisma() {
  ...
  // Triangle: top vertex (670,110), base (638,178) (702,178)
  const tx = 670, ty = 110;
  const bl = { x: 638, y: 178 }, br = { x: 702, y: 178 };

  // Light rays diverging from top vertex
  const rays = [
    { x2: 670, y2: 76 },              // straight up
    { x2: 656, y2: 78 },              // slight left
    { x2: 641, y2: 83 },              // more left
    { x2: 629, y2: 92 },              // far left
    { x2: 684, y2: 78 },              // slight right
    { x2: 698, y2: 83 },              // more right
    { x2: 710, y2: 92 },              // far right
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0f0f1a" }}>
      <svg viewBox="0 0 730 296" style={{ width: "90%", maxWidth: 660 }}>
        ...
        {/* Rays from top vertex */}
        {rays.map((r, i) => (
          <line key={i} x1={tx} y1={ty} x2={r.x2} y2={r.y2}
            stroke="white" strokeWidth={i === 0 ? 1.5 : 1}
            strokeLinecap="round"
            opacity={i === 0 ? 0.9 : 0.5 - i * 0.04}
          />
        ))}

        {/* Triangle outline */}
        <polygon
          points={`${tx},${ty} ${bl.x},${bl.y} ${br.x},${br.y}`}
          fill="none" stroke="white" strokeWidth={3} strokeLinejoin="round"
          opacity={0.9}
        />

        {/* Light dot at apex */}
        <circle cx={tx} cy={ty} r={4} fill="white" opacity={0.95} />
      </svg>
    </div>
  );
}
