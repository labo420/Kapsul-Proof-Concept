export function Angolo() {
  ...
  const cx = 670, cy = 148;
  const hw = 42, hh = 54; // half-width, half-height of the viewfinder box
  const arm = 20; // bracket arm length
  ...
  // Bracket paths: for each corner, draw two arms
  const brackets = [
    // top-left: right arm + down arm
    `M${corners[0].x + arm},${corners[0].y} L${corners[0].x},${corners[0].y} L${corners[0].x},${corners[0].y + arm}`,
    // top-right: left arm + down arm
    `M${corners[1].x - arm},${corners[1].y} L${corners[1].x},${corners[1].y} L${corners[1].x},${corners[1].y + arm}`,
    // bottom-left: right arm + up arm
    `M${corners[2].x + arm},${corners[2].y} L${corners[2].x},${corners[2].y} L${corners[2].x},${corners[2].y - arm}`,
    // bottom-right: left arm + up arm
    `M${corners[3].x - arm},${corners[3].y} L${corners[3].x},${corners[3].y} L${corners[3].x},${corners[3].y - arm}`,
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0f0f1a" }}>
      <svg viewBox="0 0 730 296" style={{ width: "90%", maxWidth: 660 }}>
        ...
        {/* 4 corner brackets */}
        {brackets.map((d, i) => (
          <path key={i} d={d} fill="none" stroke="white" strokeWidth={sw} strokeLinecap="square" opacity={0.9} />
        ))}

        {/* Center crosshair dot */}
        <circle cx={cx} cy={cy} r={10} fill="none" stroke="white" strokeWidth={1.5} opacity={0.4} />
