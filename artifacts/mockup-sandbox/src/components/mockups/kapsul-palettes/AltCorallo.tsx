// A · Corallo — stesso layout Rosa Antico ma con crimson vivace al posto del borgogna
export function AltCorallo() {
  const indigo = "#4c1d95";
  const primary = "#e11d48";
  const primaryDeep = "#9f1239";
  const accent = "#fda4af";
  const labelColor = "#fecdd3";
  const textMuted = "#ffe4e6";

  return (
    <div style={{ width: 390, minHeight: 844, background: "#08010A", fontFamily: "'Inter', sans-serif", position: "relative", overflow: "hidden", color: "#fff" }}>
      ...
      <div style={{ margin: "16px 20px 0", background: `rgba(225,29,72,0.05)`, backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)", borderRadius: 24, border: `1px solid rgba(253,164,175,0.16)`, padding: "22px 22px 20px", boxShadow: `0 8px 40px rgba(0,0,0,0.6), inset 0 1px 0 rgba(254,205,211,0.05)` }}>
      ...
      <div style={{ margin: "20px 24px 0", display: "flex", alignItems: "center", gap: 10 }}>
        {["#08010A","#4c1d95","#9f1239","#e11d48","#f43f5e","#fecdd3"].map((c, i) => (
        ...
        <span style={{ fontSize: 10, fontWeight: 500, letterSpacing: 2, color: accent, textTransform: "uppercase", marginLeft: 4 }}>A · Corallo</span>
      </div>
    </div>
  );
}
