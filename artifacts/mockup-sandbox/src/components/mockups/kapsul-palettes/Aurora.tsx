// Ametista — deep violet + lavender (vicino all'indigo ma senza il rosa Instagram)
export function Aurora() {
  const glow1 = "#4c1d9533";
  const glow2 = "#7c3aed22";
  const primary = "#7c3aed";
  const primaryDeep = "#4c1d95";
  const accent = "#c4b5fd";
  const labelColor = "#a78bfa";
  const textMuted = "#ddd6fe";
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", top: -120, left: -80, width: 400, height: 400, background: `radial-gradient(circle, ${glow1} 0%, transparent 65%)`, pointerEvents: "none" }} />
      <div style={{ position: "absolute", top: 280, right: -80, width: 300, height: 300, background: `radial-gradient(circle, ${glow2} 0%, transparent 65%)`, pointerEvents: "none" }} />
      <div style={{ position: "absolute", bottom: 0, left: "15%", width: 260, height: 260, background: `radial-gradient(circle, #6d28d922 0%, transparent 65%)`, pointerEvents: "none" }} />
      ...
      <div style={{ padding: "36px 0 0", textAlign: "center" }}>
        <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          <div style={{ width: 54, height: 54, borderRadius: 16, background: `linear-gradient(135deg, ${primaryDeep}, ${primary}, #a855f7)`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 0 30px ${primary}55, 0 0 60px ${primaryDeep}22` }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, fontWeight: 400, letterSpacing: 4, color: primary, textTransform: "uppercase", marginTop: 3 }}>moments preserved</div>
          </div>
        </div>
      </div>

      <div style={{ margin: "28px 20px 0", borderRadius: 20, height: 160, position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, background: `linear-gradient(120deg, #2e1065, ${primaryDeep}, ${primary})`, opacity: 0.95 }} />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent 40%, rgba(5,2,16,0.7))" }} />
      ...
      <div style={{ margin: "16px 20px 0", background: `rgba(124,58,237,0.05)`, backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)", borderRadius: 24, border: `1px solid rgba(167,139,250,0.18)`, padding: "22px 22px 20px", boxShadow: `0 8px 40px rgba(0,0,0,0.6), inset 0 1px 0 rgba(196,181,253,0.06)` }}>
        ...
        {["Email","Password"].map((label, i) => (
          <div key={label} style={{ marginBottom: i === 0 ? 12 : 18 }}>
            <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: "13px 16px", border: `1px solid rgba(167,139,250,0.14)` }}>
              <div style={{ fontSize: 10, fontWeight: 500, letterSpacing: 2, color: labelColor, textTransform: "uppercase", marginBottom: 4 }}>{label}</div>
              <div style={{ fontSize: 14, fontWeight: 300, color: "#6b7280" }}>{i === 0 ? "nome@email.com" : "••••••••••"}</div>
            </div>
          </div>
        ))}
        <button style={{ width: "100%", padding: "15px", borderRadius: 14, border: "none", background: `linear-gradient(90deg, ${primaryDeep}, ${primary}, #a855f7)`, color: "#fff", fontWeight: 400, fontSize: 13, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", boxShadow: `0 4px 20px ${primary}40` }}>Entra</button>
      ...
      <div style={{ margin: "20px 24px 0", display: "flex", alignItems: "center", gap: 10 }}>
        {["#050210","#2e1065","#4c1d95","#7c3aed","#a78bfa","#ddd6fe"].map((c, i) => (
        ...
        <span style={{ fontSize: 10, fontWeight: 500, letterSpacing: 2, color: primary, textTransform: "uppercase", marginLeft: 4 }}>Ametista · Viola</span>
      </div>
    </div>
  );
}
