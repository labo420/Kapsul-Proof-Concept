  ...
  return (
    <div style={{ width: 390, minHeight: 844, background: bg, fontFamily: "Inter, sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ textAlign: "center", padding: "40px 0 10px" }}>
        ...
        <p style={{ color: "#6EE7B7", fontSize: 13, marginTop: 6 }}>Cattura i tuoi momenti</p>
      </div>
      ...
      <div style={{ margin: "20px 20px 0", background: card, borderRadius: 20, border: `1px solid ${border}`, padding: 20 }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <label style={{ color: "#6EE7B7", fontSize: 12, fontWeight: 600, display: "block", marginBottom: 6 }}>Email</label>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ color: "#6EE7B7", fontSize: 12, fontWeight: 600, display: "block", marginBottom: 6 }}>Password</label>
          <div style={{ background: muted, borderRadius: 12, padding: "12px 14px", border: `1px solid ${border}` }}>
        ...
        <p style={{ textAlign: "center", color: "#6EE7B7", fontSize: 13, marginTop: 14 }}>Non hai un account? <span style={{ color: accent, fontWeight: 700 }}>Registrati</span></p>
      </div>

      {/* Palette chips */}
      <div style={{ margin: "24px 20px 0", display: "flex", gap: 8, alignItems: "center" }}>
        <span style={{ color: "#4B7D69", fontSize: 11, fontWeight: 600 }}>PALETTE</span>
        {["#030D0A","#081A14","#10B981","#06D6A0","#6EE7B7"].map((c, i) => (
          <div key={i} style={{ width: 24, height: 24, borderRadius: "50%", background: c, border: "2px solid rgba(255,255,255,0.15)" }} />
        ))}
        <span style={{ color: "#6EE7B7", fontSize: 11, fontWeight: 700, marginLeft: 4 }}>Aurora · Smeraldo</span>
      </div>
    </div>
  );
}
