// B · Magenta — deep hot magenta, più caldo e vivace del borgogna
export function AltMagenta() {
  const indigo = "#4c1d95";
  const primary = "#db2777";
  const primaryDeep = "#9d174d";
  const accent = "#f9a8d4";
  const labelColor = "#fbcfe8";
  const textMuted = "#fce7f3";

  return (
    <div style={{ width: 390, minHeight: 844, background: "#080009", fontFamily: "'Inter', sans-serif", position: "relative", overflow: "hidden", color: "#fff" }}>
      ...
      <div style={{ margin: "28px 20px 0", borderRadius: 20, height: 160, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent 40%, rgba(8,0,9,0.7))" }} />
      ...
      <div style={{ margin: "16px 20px 0", background: `rgba(219,39,119,0.05)`, backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)", borderRadius: 24, border: `1px solid rgba(249,168,212,0.16)`, padding: "22px 22px 20px", boxShadow: `0 8px 40px rgba(0,0,0,0.6), inset 0 1px 0 rgba(251,207,232,0.05)` }}>
        ...
        {["Email","Password"].map((label, i) => (
          <div key={label} style={{ marginBottom: i === 0 ? 12 : 18 }}>
            <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: "13px 16px", border: "1px solid rgba(249,168,212,0.12)" }}>
      ...
      <div style={{ margin: "20px 24px 0", display: "flex", alignItems: "center", gap: 10 }}>
        {["#080009","#4c1d95","#9d174d","#db2777","#ec4899","#fbcfe8"].map((c, i) => (
        ...
        <span style={{ fontSize: 10, fontWeight: 500, letterSpacing: 2, color: accent, textTransform: "uppercase", marginLeft: 4 }}>B · Magenta</span>
      </div>
    </div>
  );
}
