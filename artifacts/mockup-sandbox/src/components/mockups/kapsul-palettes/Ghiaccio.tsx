// Lapislazzuli — royal indigo + champagne gold (lontano da Instagram, molto lusso)
export function Ghiaccio() {
  const primary = "#4338ca";
  const primaryDeep = "#312e81";
  const accent = "#fbbf24";
  const labelColor = "#fde68a";
  const textMuted = "#ede9fe";
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", top: 280, right: -80, width: 300, height: 300, background: "radial-gradient(circle, #92400e22 0%, transparent 65%)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", bottom: 0, left: "15%", width: 260, height: 260, background: "radial-gradient(circle, #1e1b4b22 0%, transparent 65%)", pointerEvents: "none" }} />
      ...
      <div style={{ padding: "36px 0 0", textAlign: "center" }}>
        <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          <div style={{ width: 54, height: 54, borderRadius: 16, background: `linear-gradient(135deg, ${primaryDeep}, ${primary}, #6366f1)`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 0 30px ${primary}55, 0 0 60px #92400e22` }}>
      ...
      <div style={{ margin: "28px 20px 0", borderRadius: 20, height: 160, position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, background: `linear-gradient(120deg, #1e1b4b, ${primaryDeep}, ${primary})`, opacity: 0.95 }} />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent 40%, rgba(2,1,16,0.7))" }} />
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "16px 20px" }}>
          <div style={{ fontSize: 13, fontWeight: 300, letterSpacing: 3, textTransform: "uppercase", color: labelColor, marginBottom: 4 }}>Estate 2025</div>
        ...
        <div style={{ position: "absolute", top: 16, right: 20, display: "flex" }}>
          ...
          <div style={{ width: 28, height: 28, borderRadius: "50%", background: accent, border: `1.5px solid ${accent}88`, marginLeft: -8, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 10, color: "#020110", fontWeight: 700 }}>+5</span>
          </div>
        </div>
      </div>

      <div style={{ margin: "16px 20px 0", background: `rgba(67,56,202,0.05)`, backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)", borderRadius: 24, border: `1px solid rgba(99,102,241,0.18)`, padding: "22px 22px 20px", boxShadow: `0 8px 40px rgba(0,0,0,0.6), inset 0 1px 0 rgba(251,191,36,0.07)` }}>
      ...
      <div style={{ margin: "20px 24px 0", display: "flex", alignItems: "center", gap: 10 }}>
        {["#020110","#1e1b4b","#312e81","#4338ca","#6366f1","#fbbf24"].map((c, i) => (
        ...
        <span style={{ fontSize: 10, fontWeight: 500, letterSpacing: 2, color: accent, textTransform: "uppercase", marginLeft: 4 }}>Lapislazzuli · Oro</span>
      </div>
    </div>
  );
}
