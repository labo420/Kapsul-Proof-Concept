// D · Rame — indigo + rame/terracotta caldo (contrasto inaspettato, molto premium)
export function AltRame() {
  const indigo = "#4c1d95";
  const primary = "#c2410c";
  const primaryDeep = "#9a3412";
  const accent = "#fdba74";
  const labelColor = "#fed7aa";
  const textMuted = "#ffedd5";

  return (
    <div style={{ width: 390, minHeight: 844, background: "#070208", fontFamily: "'Inter', sans-serif", position: "relative", overflow: "hidden", color: "#fff" }}>
      ...
      <div style={{ margin: "28px 20px 0", borderRadius: 20, height: 160, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent 40%, rgba(7,2,8,0.7))" }} />
      ...
      <div style={{ margin: "16px 20px 0", background: `rgba(194,65,12,0.05)`, backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)", borderRadius: 24, border: `1px solid rgba(253,186,116,0.16)`, padding: "22px 22px 20px", boxShadow: `0 8px 40px rgba(0,0,0,0.6), inset 0 1px 0 rgba(254,215,170,0.05)` }}>
        ...
        {["Email","Password"].map((label, i) => (
          <div key={label} style={{ marginBottom: i === 0 ? 12 : 18 }}>
            <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: "13px 16px", border: "1px solid rgba(253,186,116,0.12)" }}>
      ...
      <div style={{ margin: "20px 24px 0", display: "flex", alignItems: "center", gap: 10 }}>
        {["#070208","#4c1d95","#9a3412","#c2410c","#ea580c","#fed7aa"].map((c, i) => (
        ...
        <span style={{ fontSize: 10, fontWeight: 500, letterSpacing: 2, color: accent, textTransform: "uppercase", marginLeft: 4 }}>D · Rame</span>
      </div>
    </div>
  );
}
