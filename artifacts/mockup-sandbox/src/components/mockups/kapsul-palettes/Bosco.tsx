// Rosa Antico — indigo profondo + rose scuro/borgogna (non il rosa neon di Instagram)
export function Bosco() {
  const primary = "#be123c";
  const primaryDeep = "#881337";
  const accent = "#fda4af";
  const labelColor = "#fecdd3";
  const textMuted = "#ffe4e6";
  const indigo = "#4c1d95";
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", top: -120, left: -80, width: 400, height: 400, background: "radial-gradient(circle, #88133733 0%, transparent 65%)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", top: 280, right: -80, width: 300, height: 300, background: "radial-gradient(circle, #4c1d9522 0%, transparent 65%)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", bottom: 0, left: "15%", width: 260, height: 260, background: "radial-gradient(circle, #9f123922 0%, transparent 65%)", pointerEvents: "none" }} />
      ...
      <div style={{ margin: "28px 20px 0", borderRadius: 20, height: 160, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent 40%, rgba(8,1,10,0.7))" }} />
      ...
      <div style={{ margin: "16px 20px 0", background: `rgba(190,18,60,0.05)`, backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)", borderRadius: 24, border: `1px solid rgba(253,164,175,0.16)`, padding: "22px 22px 20px", boxShadow: `0 8px 40px rgba(0,0,0,0.6), inset 0 1px 0 rgba(254,205,211,0.05)` }}>
        ...
        {["Email","Password"].map((label, i) => (
          <div key={label} style={{ marginBottom: i === 0 ? 12 : 18 }}>
            <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: "13px 16px", border: "1px solid rgba(253,164,175,0.12)" }}>
      ...
      <div style={{ margin: "20px 24px 0", display: "flex", alignItems: "center", gap: 10 }}>
        {["#08010A","#4c1d95","#881337","#be123c","#fb7185","#fecdd3"].map((c, i) => (
        ...
        <span style={{ fontSize: 10, fontWeight: 500, letterSpacing: 2, color: accent, textTransform: "uppercase", marginLeft: 4 }}>Rosa Antico · Borgogna</span>
      </div>
    </div>
  );
}
